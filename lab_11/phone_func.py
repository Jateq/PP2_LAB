...
def pattern(pt, method):
    if method == 1:
        cur.execute(f'''SELECT "NAME","NUMBER" FROM phonebook WHERE "NAME" LIKE '{pt}%' ''')
    if method == 2:
        cur.execute(f'''SELECT "NAME", "NUMBER" FROM phonebook WHERE "NAME" LIKE '{pt}%' ''')
    if method == 3:
        cur.execute(f'''SELECT "NAME", "NUMBER" FROM phonebook WHERE "NUMBER" LIKE '{pt}%' ''')                
    rows = cur.fetchall()
    for i in rows:
        print(i) 

def add(new_user):
    cur.execute('''SELECT id, "NAME", "NUMBER" from PHONEBOOK''')
    data = cur.fetchall()
    for d in data:
        if int(d[0]) == int(new_user[0]):
            cur.execute(f"DELETE from PHONEBOOK where id={new_user[0]}")                                
            cur.execute(f'''INSERT INTO PHONEBOOK (ID, "NAME", "NUMBER") VALUES({new_user[0]}, '{new_user[1]}', '{new_user[2]}', '{new_user[3]}')''')
            con.commit()    
